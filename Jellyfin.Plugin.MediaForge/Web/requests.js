export default function (view, params) {
  ...
  q('diagnostics').onclick = async () => { try { const data = await call('Admin/Diagnostics'); const host = q('diagnostic-result'); host.hidden = false; host.textContent = 'Plugin: ' + data.pluginVersion + ' · Verbindung: ' + (data.connection.healthy ? 'OK' : 'nicht verfügbar') + ' · Modul: ' + (data.module?.version || 'unbekannt') + ' · Fähigkeiten: ' + (data.module?.capabilities || []).join(', ') + ' · Autosync-Funktion: ' + (data.module?.autosync_runtime?.handler_available === true ? 'geladen' : data.module?.autosync_runtime?.handler_available === false ? 'fehlt – MediaForge neu starten' : 'unbekannt') + ' · Core-Aufruf: ' + (data.module?.autosync_runtime?.uses_raw_views === true ? 'MediaForge-Modul-API' : 'Kompatibilitätsweg') + ' · Berechtigungen: ' + JSON.stringify(data.module?.permissions || {}) + ((data.module?.capabilities || []).includes('autosync') ? '' : ' · MediaForge-Modul für Autosync aktualisieren.'); } catch (error) { notice(error.message, true); } };
